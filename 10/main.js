...
var $author$project$Main$testTree = A2(
...
function $author$project$Main$cyclic$treeDecoder() {
	return A4(
			...
			A2(
				...
				$elm$json$Json$Decode$list(
					$elm$json$Json$Decode$lazy(
						function (_v0) {
							return $author$project$Main$cyclic$treeDecoder();
						})))));
}
try {
	var $author$project$Main$treeDecoder = $author$project$Main$cyclic$treeDecoder();
	$author$project$Main$cyclic$treeDecoder = function () {
		return $author$project$Main$treeDecoder;
	};
} catch ($) {
	throw 'Some top-level definitions from `Main` are causing infinite recursion:\n\n  ┌─────┐\n  │    treeDecoder\n  └─────┘\n\nThese errors are very tricky, so read https://elm-lang.org/0.19.1/bad-recursion to learn how to fix it!';}
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{flare: $elm$core$Maybe$Nothing, statusMsg: 'Lade flare.json ...', testTree: $author$project$Main$testTree},
		$elm$http$Http$get(
			{
				expect: A2($elm$http$Http$expectJson, $author$project$Main$GotFlare, $author$project$Main$treeDecoder),
				url: 'flare.json'
			}));
};
...
var $author$project$Main$computeSums = function (tree) {
	var ownValue = $gampleman$elm_rosetree$Tree$label(tree).b;
	var children = A2(
		$elm$core$List$map,
		$author$project$Main$computeSums,
		$gampleman$elm_rosetree$Tree$children(tree));
	var childSum = $elm$core$List$sum(
		A2(
			$elm$core$List$map,
			A2($elm$core$Basics$composeR, $gampleman$elm_rosetree$Tree$label, $elm$core$Tuple$second),
			children));
	return A2(
		$gampleman$elm_rosetree$Tree$tree,
		_Utils_Tuple2(
			$gampleman$elm_rosetree$Tree$label(tree).a,
			ownValue + childSum),
		children);
};
var $author$project$Main$httpErrorToString = function (error) {
	switch (error.$) {
		...
		case 'NetworkError':
			return 'flare.json konnte nicht geladen werden. Starte die Seite ueber einen lokalen Webserver im Ordner uebung/10.';
		case 'BadStatus':
			var status = error.a;
		...
		if (msg.a.$ === 'Ok') {
			...
			var numericTree = A2(
				...
				function (_v1) {
					...
					return _Utils_Tuple2(
						name,
						A2($elm$core$Maybe$withDefault, 0, value));
				},
				newTree);
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						flare: $elm$core$Maybe$Just(
							$author$project$Main$computeSums(numericTree)),
						statusMsg: 'flare.json geladen'
					}),
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						flare: $elm$core$Maybe$Nothing,
						statusMsg: $author$project$Main$httpErrorToString(error)
					}),
				$elm$core$Platform$Cmd$none);
		}
	});
var $author$project$Main$ColorByLevel1 = {$: 'ColorByLevel1'};
var $author$project$Main$ColorByLevel2 = {$: 'ColorByLevel2'};
...
var $author$project$Main$drawTreeNode = F6(
	function (splitX, x, y, width, height, tree) {
		var nodeRectangle = A2(
			...
			_List_fromArray(
				[
					...
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$width(width),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$height(height),
					$elm_community$typed_svg$TypedSvg$Attributes$class(
					_List_fromArray(
						['simple-rect']))
				]),
			_List_Nil);
		var children = $gampleman$elm_rosetree$Tree$children(tree);
		var childTotal = $elm$core$List$sum(
			A2(
				$elm$core$List$map,
				A2($elm$core$Basics$composeR, $gampleman$elm_rosetree$Tree$label, $elm$core$Tuple$second),
				children));
		var childRectangles = ($elm$core$List$isEmpty(children) || (childTotal <= 0)) ? _List_Nil : A3(
			$elm$core$List$foldl,
			F2(
				function (child, _v0) {
					var offset = _v0.a;
					var acc = _v0.b;
					var nextY = splitX ? y : offset;
					var nextX = splitX ? offset : x;
					var childValue = $gampleman$elm_rosetree$Tree$label(child).b;
					var childSize = childValue / childTotal;
					var nextHeight = splitX ? height : (height * childSize);
					var nextWidth = splitX ? (width * childSize) : width;
					var nextOffset = splitX ? (offset + nextWidth) : (offset + nextHeight);
					return _Utils_Tuple2(
						nextOffset,
						_Utils_ap(
							acc,
							A6($author$project$Main$drawTreeNode, !splitX, nextX, nextY, nextWidth, nextHeight, child)));
				}),
			_Utils_Tuple2(
				splitX ? x : y,
				_List_Nil),
			children).b;
		return A2($elm$core$List$cons, nodeRectangle, childRectangles);
	});
...
var $author$project$Main$drawTreemapSimple = function (tree) {
	var padding = 16;
	var canvasWidth = 760;
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					['treemap-svg'])),
				A4($elm_community$typed_svg$TypedSvg$Attributes$viewBox, 0, 0, canvasWidth + (2 * padding), canvasHeight + (2 * padding))
			]),
		_List_fromArray(
			[
				A2(
				...
				A6($author$project$Main$drawTreeNode, true, 0, 0, canvasWidth, canvasHeight, tree))
			]));
};
...
var $author$project$Main$labelIfRoom = function (node) {
	var name = node.node.a;
	return ((node.width > 82) && ((node.height > 26) && ($elm$core$String$length(name) < 18))) ? _List_fromArray(
		[
			A2(
			$elm_community$typed_svg$TypedSvg$text_,
			_List_fromArray(
				[
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$x(node.x + 4),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(node.y + 15),
					$elm_community$typed_svg$TypedSvg$Attributes$class(
					_List_fromArray(
						['map-label']))
				]),
...
var $author$project$Main$rootBorder = function (node) {
	return A2(
		$elm_community$typed_svg$TypedSvg$rect,
		_List_fromArray(
			[
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$x(node.x),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(node.y),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$width(node.width),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$height(node.height),
				$elm_community$typed_svg$TypedSvg$Attributes$class(
				_List_fromArray(
					['root-border']))
			]),
...
var $author$project$Main$itemAt = F2(
	function (index, list) {
...
var $author$project$Main$hashString = function (value) {
	return A3(
		$elm$core$List$foldl,
		F2(
			function (_char, acc) {
				return acc + $elm$core$Char$toCode(_char);
			}),
		0,
		$elm$core$String$toList(value));
};
var $elm$core$Basics$modBy = _Basics_modBy;
var $author$project$Main$paletteIndex = function (key) {
	return A2(
		$elm$core$Basics$modBy,
		12,
		$author$project$Main$hashString(key));
};
var $author$project$Main$colorClassForNode = F3(
	function (mode, ancestors, node) {
		var path = A2(
			$elm$core$List$map,
			A2(
			...
			$elm$core$List$reverse(
				A2($elm$core$List$cons, node, ancestors)));
		var key = function () {
			if (mode.$ === 'ColorByLevel1') {
				return A2(
					$elm$core$Maybe$withDefault,
					'root',
					A2($author$project$Main$itemAt, 1, path));
			} else {
				return A2(
					$elm$core$Maybe$withDefault,
					A2(
						...
						'root',
						A2($author$project$Main$itemAt, 1, path)),
					A2($author$project$Main$itemAt, 2, path));
			}
		}();
		return 'color-' + $elm$core$String$fromInt(
			$author$project$Main$paletteIndex(key));
	});
var $author$project$Main$squarifiedRect = F3(
	function (mode, ancestors, node) {
		return A2(
			$elm_community$typed_svg$TypedSvg$rect,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							'squarified-rect',
							A3($author$project$Main$colorClassForNode, mode, ancestors, node)
						]))
				]),
			_List_Nil);
	});
var $author$project$Main$squarifiedNodes = F2(
	function (mode, tree) {
		return $elm$core$List$reverse(
			A3(
				$gampleman$elm_rosetree$Tree$depthFirstFold,
				F4(
					function (acc, ancestors, node, _v0) {
						return $gampleman$elm_rosetree$Tree$Continue(
							function () {
								if (!ancestors.b) {
									return A2(
										$elm$core$List$cons,
										$author$project$Main$rootBorder(node),
										acc);
								} else {
									return A2(
										$elm$core$List$cons,
										A3($author$project$Main$squarifiedRect, mode, ancestors, node),
										_Utils_ap(
											$author$project$Main$labelIfRoom(node),
											acc));
								}
							}());
					}),
...
var $author$project$Main$drawTreemapSquarified = F2(
	function (mode, tree) {
		var padding = 12;
		var canvasWidth = 920;
		var canvasHeight = 620;
		var positionedTree = A3(
			$gampleman$elm_visualization$Hierarchy$treemap,
			_List_fromArray(
				[
					A2($gampleman$elm_visualization$Hierarchy$size, canvasWidth, canvasHeight),
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2($author$project$Main$squarifiedNodes, mode, positionedTree))
				]));
	});
...
var $author$project$Main$sectionView = F2(
...
var $author$project$Main$labelToHtml = function (_v0) {
	var label = _v0.a;
	var value = _v0.b;
	return $elm$html$Html$text(
		label + (' ' + $elm$core$String$fromInt(value)));
};
var $gampleman$elm_rosetree$Tree$restructure = F3(
	function (convertLabel, convertTree, t) {
		return A4(
			$gampleman$elm_rosetree$Tree$depthFirstTraversal,
			F4(
				function (s, _v0, l, c) {
...
var $author$project$Main$toListItem = F2(
...
var $author$project$Main$treeAsHtml = function (tree) {
	return function (root) {
		return A2(
			$elm$html$Html$ul,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('tree-list')
				]),
			_List_fromArray(
				[root]));
	}(
		A3($gampleman$elm_rosetree$Tree$restructure, $author$project$Main$labelToHtml, $author$project$Main$toListItem, tree));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Uebung 10: Treemaps')
					])),
				A2(
				$author$project$Main$sectionView,
				'10.1 Einfache Treemap - testTree',
				_List_fromArray(
					[
						$author$project$Main$drawTreemapSimple(model.testTree)
					])),
				function () {
				var _v0 = model.flare;
				if (_v0.$ === 'Just') {
					...
					return A2(
						...
						_List_fromArray(
							[
								A2(
								$author$project$Main$sectionView,
								'10.1 Einfache Treemap - flare.json',
								_List_fromArray(
									[
										$author$project$Main$drawTreemapSimple(flareTree)
									])),
								A2(
								$author$project$Main$sectionView,
								'10.2 Squarified Treemap - Farben ab Ebene 1',
								_List_fromArray(
									[
										A2($author$project$Main$drawTreemapSquarified, $author$project$Main$ColorByLevel1, flareTree)
									])),
								A2(
								$author$project$Main$sectionView,
								'10.2 Squarified Treemap - Farben ab Ebene 2',
								_List_fromArray(
									[
										A2($author$project$Main$drawTreemapSquarified, $author$project$Main$ColorByLevel2, flareTree)
									])),
								A2(
								$author$project$Main$sectionView,
								'Kontrollausgabe',
								_List_fromArray(
									[
										$author$project$Main$treeAsHtml(flareTree)
									]))
							]));
				} else {
					return A2(
