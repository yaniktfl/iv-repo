...
var $author$project$Main$BinaryTree = {$: 'BinaryTree'};
var $author$project$Main$GotCountries = function (a) {
	return {$: 'GotCountries', a: a};
};
var $author$project$Main$GotFlare = function (a) {
	return {$: 'GotFlare', a: a};
};
...
var $author$project$Main$mkNode = F2(
	function (label, value) {
		return {id: 0, label: label, value: value};
	});
...
function $author$project$Main$cyclic$countryDecoder() {
	return A3(
		...
		F2(
			function (name, children) {
				return A2(
					...
					A2($author$project$Main$mkNode, name, $elm$core$Maybe$Nothing),
					A2($elm$core$Maybe$withDefault, _List_Nil, children));
			}),
		A2(
			...
			A2(
				...
				$elm$json$Json$Decode$list(
					$elm$json$Json$Decode$lazy(
						function (_v0) {
							return $author$project$Main$cyclic$countryDecoder();
						})))));
}
try {
	var $author$project$Main$countryDecoder = $author$project$Main$cyclic$countryDecoder();
	$author$project$Main$cyclic$countryDecoder = function () {
		return $author$project$Main$countryDecoder;
	};
} catch ($) {
...
function $author$project$Main$cyclic$flareDecoder() {
	return A4(
		$elm$json$Json$Decode$map3,
		F3(
			function (name, value, children) {
				return A2(
					$gampleman$elm_rosetree$Tree$tree,
					A2($author$project$Main$mkNode, name, value),
					A2($elm$core$Maybe$withDefault, _List_Nil, children));
			}),
		...
		$elm$json$Json$Decode$maybe(
			A2(
				...
				$elm$json$Json$Decode$list(
					$elm$json$Json$Decode$lazy(
						function (_v0) {
							return $author$project$Main$cyclic$flareDecoder();
						})))));
}
try {
	var $author$project$Main$flareDecoder = $author$project$Main$cyclic$flareDecoder();
	$author$project$Main$cyclic$flareDecoder = function () {
		return $author$project$Main$flareDecoder;
	};
} catch ($) {
	throw 'Some top-level definitions from `Main` are causing infinite recursion:\n\n  ┌─────┐\n  │    flareDecoder\n  └─────┘\n\nThese errors are very tricky, so read https://elm-lang.org/0.19.1/bad-recursion to learn how to fix it!';}
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{
			active: $author$project$Main$BinaryTree,
			countries: $author$project$Main$Loading,
			flare: $author$project$Main$Loading,
			hovered: $elm$core$Maybe$Just(0)
		},
		$elm$core$Platform$Cmd$batch(
			_List_fromArray(
				[
					$elm$http$Http$get(
					{
						expect: A2($elm$http$Http$expectJson, $author$project$Main$GotFlare, $author$project$Main$flareDecoder),
						url: 'flare.json'
					}),
					$elm$http$Http$get(
					{
						expect: A2($elm$http$Http$expectJson, $author$project$Main$GotCountries, $author$project$Main$countryDecoder),
						url: 'countryHierarchy.json'
					})
				])));
};
...
var $author$project$Main$assignIds = $gampleman$elm_rosetree$Tree$indexedMap(
	F2(
		function (id, node) {
...
var $author$project$Main$httpErrorToString = function (error) {
	switch (error.$) {
		...
		case 'Timeout':
			return 'Zeitueberschreitung beim Laden der Datei.';
		case 'NetworkError':
			return 'Die Datei konnte nicht geladen werden. Starte die Seite ueber einen lokalen Webserver im Ordner uebung/9.';
		case 'BadStatus':
			var status = error.a;
			return 'HTTP-Fehler ' + $elm$core$String$fromInt(status);
		default:
			var message = error.a;
			return 'JSON-Decoder-Fehler: ' + message;
	}
};
		...
		switch (msg.$) {
			case 'Select':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{active: exercise, hovered: $elm$core$Maybe$Nothing}),
			...
			case 'Hover':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							hovered: $elm$core$Maybe$Just(id)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Leave':
				return _Utils_Tuple2(
				...
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flare: $author$project$Main$Loaded(
									$author$project$Main$assignIds(tree))
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flare: $author$project$Main$Failed(
				...
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								countries: $author$project$Main$Loaded(
									$author$project$Main$assignIds(tree))
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								countries: $author$project$Main$Failed(
									$author$project$Main$httpErrorToString(error))
							}),
						$elm$core$Platform$Cmd$none);
				}
...
var $author$project$Main$CountryTree = {$: 'CountryTree'};
var $author$project$Main$FlareTree = {$: 'FlareTree'};
var $author$project$Main$OrderedTree = {$: 'OrderedTree'};
var $gampleman$elm_rosetree$Tree$singleton = function (v) {
	return A2($gampleman$elm_rosetree$Tree$Tree, v, _List_Nil);
};
var $author$project$Main$leaf = function (label) {
	return $gampleman$elm_rosetree$Tree$singleton(
		A2($author$project$Main$mkNode, label, $elm$core$Maybe$Nothing));
};
var $author$project$Main$binaryTree = $author$project$Main$assignIds(
	A2(
		$gampleman$elm_rosetree$Tree$tree,
		A2($author$project$Main$mkNode, 'root', $elm$core$Maybe$Nothing),
		_List_fromArray(
			[
				A2(
				$gampleman$elm_rosetree$Tree$tree,
				A2($author$project$Main$mkNode, 'left', $elm$core$Maybe$Nothing),
				_List_fromArray(
					[
						A2(
						$gampleman$elm_rosetree$Tree$tree,
						A2($author$project$Main$mkNode, 'left.left', $elm$core$Maybe$Nothing),
						_List_fromArray(
							[
								A2(
								$gampleman$elm_rosetree$Tree$tree,
								A2($author$project$Main$mkNode, 'left.left.left', $elm$core$Maybe$Nothing),
								_List_fromArray(
									[
										$author$project$Main$leaf('left.left.left.left')
									]))
							])),
						A2(
						$gampleman$elm_rosetree$Tree$tree,
						A2($author$project$Main$mkNode, 'left.right', $elm$core$Maybe$Nothing),
						_List_fromArray(
							[
								A2(
								$gampleman$elm_rosetree$Tree$tree,
								A2($author$project$Main$mkNode, 'user12', $elm$core$Maybe$Nothing),
								_List_fromArray(
									[
										A2(
										$gampleman$elm_rosetree$Tree$tree,
										A2($author$project$Main$mkNode, 'user11', $elm$core$Maybe$Nothing),
										_List_fromArray(
											[
												$author$project$Main$leaf('user10')
											])),
										$author$project$Main$leaf('user13')
									]))
							]))
					])),
				A2(
				$gampleman$elm_rosetree$Tree$tree,
				A2($author$project$Main$mkNode, 'right', $elm$core$Maybe$Nothing),
				_List_fromArray(
					[
						$author$project$Main$leaf('user2'),
						A2(
						$gampleman$elm_rosetree$Tree$tree,
						A2($author$project$Main$mkNode, 'right.right', $elm$core$Maybe$Nothing),
						_List_fromArray(
							[
								$author$project$Main$leaf('right.right.child')
							]))
					]))
			])));
...
var $elm_community$typed_svg$TypedSvg$Types$DominantBaselineAuto = {$: 'DominantBaselineAuto'};
...
var $author$project$Main$hoverText = function (node) {
	var _v0 = node.value;
	if (_v0.$ === 'Just') {
		var value = _v0.a;
		return node.label + (' (' + ($elm$core$String$fromInt(value) + ')'));
	} else {
		return node.label + (' ' + $elm$core$String$fromInt(node.id));
	}
};
...
var $author$project$Main$hoverLabelElement = F3(
	function (config, hovered, positioned) {
		if (_Utils_eq(
			hovered,
			$elm$core$Maybe$Just(positioned.node.id))) {
			var ty = (positioned.y - config.nodeRadius) - 12;
			var tx = positioned.x;
			var labelText = $author$project$Main$hoverText(positioned.node);
			var attrs = _List_fromArray(
				[
					...
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(ty),
					$elm_community$typed_svg$TypedSvg$Attributes$textAnchor($elm_community$typed_svg$TypedSvg$Types$AnchorMiddle),
					$elm_community$typed_svg$TypedSvg$Attributes$dominantBaseline($elm_community$typed_svg$TypedSvg$Types$DominantBaselineAuto),
...
var $author$project$Main$linkElement = function (_v0) {
	var parent = _v0.a;
	var child = _v0.b;
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					['edge'])),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$x1(parent.x),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$y1(parent.y),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$x2(child.x),
				$elm_community$typed_svg$TypedSvg$Attributes$InPx$y2(child.y),
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							'node',
							isHovered ? 'node-hovered' : 'node-default'
						])),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$cx(positioned.x),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$cy(positioned.y),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$r(config.nodeRadius),
					$elm_community$typed_svg$TypedSvg$Events$onMouseOver(
					$author$project$Main$Hover(positioned.node.id)),
					$elm_community$typed_svg$TypedSvg$Events$onMouseOut($author$project$Main$Leave)
				]),
			_List_Nil);
	});
...
var $author$project$Main$nodeLabelElement = F2(
	function (config, positioned) {
		if (config.showLeafLabels && ($elm$core$String$length(positioned.node.label) <= 60)) {
			var labelY = positioned.y + 3;
			var labelX = (positioned.x + config.nodeRadius) + 5;
			var rotation = config.rotateNodeLabels ? _List_fromArray(
				[
					$elm_community$typed_svg$TypedSvg$Attributes$transform(
					_List_fromArray(
						[
							A3($elm_community$typed_svg$TypedSvg$Types$Rotate, 90, labelX, labelY)
						]))
				]) : _List_Nil;
			return $elm$core$Maybe$Just(
				A2(
					...
					_Utils_ap(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									['node-label'])),
								$elm_community$typed_svg$TypedSvg$Attributes$InPx$x(labelX),
								$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(labelY),
					...
					_List_fromArray(
						[
							$elm_community$typed_svg$TypedSvg$Core$text(positioned.node.label)
						])));
		} else {
			return $elm$core$Maybe$Nothing;
		}
...
var $author$project$Main$treeView = F3(
	function (config, hovered, sourceTree) {
		var topPad = config.nodeRadius * 4;
		var sidePad = config.nodeRadius * 5;
		var layoutAttrs = _Utils_ap(
			function () {
				var _v1 = config.fixedSize;
				if (_v1.$ === 'Just') {
					var _v2 = _v1.a;
					var w = _v2.a;
					var h = _v2.b;
			...
			_List_fromArray(
				[
					...
					function (_v3) {
						return _Utils_Tuple2(config.nodeRadius * 2, config.nodeRadius * 2);
					}),
					$gampleman$elm_visualization$Hierarchy$parentChildMargin(config.parentChildMargin),
					$gampleman$elm_visualization$Hierarchy$peerMargin(config.peerMargin),
					$gampleman$elm_visualization$Hierarchy$layered
				]));
		var positionedTree = A2($gampleman$elm_visualization$Hierarchy$tidy, layoutAttrs, sourceTree);
		var links = $gampleman$elm_rosetree$Tree$links(positionedTree);
		var positionedNodes = $gampleman$elm_rosetree$Tree$toList(positionedTree);
		var xs = A2(
			$elm$core$List$map,
			function ($) {
				return $.x;
		...
		var vbW = (maxX - minX) + (sidePad * 2);
		var vbX = minX - sidePad;
		var ys = A2(
			$elm$core$List$map,
			function ($) {
				return $.y;
		...
		var contents = _Utils_ap(
			A2($elm$core$List$map, $author$project$Main$linkElement, links),
			_Utils_ap(
				A2(
					$elm$core$List$map,
					A2($author$project$Main$nodeElement, config, hovered),
					positionedNodes),
				_Utils_ap(
					A2(
						$elm$core$List$filterMap,
						$author$project$Main$nodeLabelElement(config),
						positionedNodes),
					A2(
						$elm$core$List$filterMap,
						A2($author$project$Main$hoverLabelElement, config, hovered),
						positionedNodes))));
		var bottomPad = config.rotateNodeLabels ? 170 : (config.nodeRadius * 4);
		var vbH = ((maxY - minY) + topPad) + bottomPad;
		var _v0 = config.fixedSize;
		if (_v0.$ === 'Just') {
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							['tree-svg'])),
						A4($elm_community$typed_svg$TypedSvg$Attributes$viewBox, vbX, vbY, vbW, vbH)
					]),
				contents);
		} else {
			return A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									['tree-svg-natural'])),
								$elm_community$typed_svg$TypedSvg$Attributes$InPx$width(vbW),
								$elm_community$typed_svg$TypedSvg$Attributes$InPx$height(vbH),
								A4($elm_community$typed_svg$TypedSvg$Attributes$viewBox, vbX, vbY, vbW, vbH)
							]),
						contents)
					]));
...
var $author$project$Main$loadedTreeView = F3(
	function (config, hovered, state) {
		switch (state.$) {
			case 'Loading':
				return A2(
					$elm$html$Html$div,
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Lade lokale JSON-Datei ...')
						]));
			case 'Failed':
				var message = state.a;
				return A2(
			...
			default:
				var tree = state.a;
				return A3($author$project$Main$treeView, config, hovered, tree);
		}
	});
var $author$project$Main$orderedTree = $author$project$Main$assignIds(
	A2(
		$gampleman$elm_rosetree$Tree$tree,
		A2($author$project$Main$mkNode, 'root', $elm$core$Maybe$Nothing),
		_List_fromArray(
			[
				A2(
				$gampleman$elm_rosetree$Tree$tree,
				A2($author$project$Main$mkNode, 'group-a', $elm$core$Maybe$Nothing),
				_List_fromArray(
					[
						$author$project$Main$leaf('a1'),
						$author$project$Main$leaf('a2'),
						$author$project$Main$leaf('a3'),
						$author$project$Main$leaf('a4'),
						$author$project$Main$leaf('a5'),
						$author$project$Main$leaf('a6'),
						A2(
						$gampleman$elm_rosetree$Tree$tree,
						A2($author$project$Main$mkNode, 'a7', $elm$core$Maybe$Nothing),
						_List_fromArray(
							[
								$author$project$Main$leaf('a7.1')
							]))
					])),
				$author$project$Main$leaf('single-b'),
				$author$project$Main$leaf('single-c'),
				A2(
				$gampleman$elm_rosetree$Tree$tree,
				A2($author$project$Main$mkNode, 'chain-d', $elm$core$Maybe$Nothing),
				_List_fromArray(
					[
						$author$project$Main$leaf('chain-d.1')
					])),
				$author$project$Main$leaf('single-e'),
				A2(
				$gampleman$elm_rosetree$Tree$tree,
				A2($author$project$Main$mkNode, 'group-f', $elm$core$Maybe$Nothing),
				_List_fromArray(
					[
						A2(
						$gampleman$elm_rosetree$Tree$tree,
						A2($author$project$Main$mkNode, '1', $elm$core$Maybe$Nothing),
						_List_fromArray(
							[
								$author$project$Main$leaf('f1'),
								$author$project$Main$leaf('f2'),
								$author$project$Main$leaf('f3'),
								$author$project$Main$leaf('f4'),
								$author$project$Main$leaf('f5'),
								$author$project$Main$leaf('f6'),
								$author$project$Main$leaf('f7')
							]))
					]))
			])));
var $author$project$Main$activeTreeView = function (model) {
	var _v0 = model.active;
	switch (_v0.$) {
		case 'BinaryTree':
			return A3(
				$author$project$Main$treeView,
				{
					fixedSize: $elm$core$Maybe$Just(
						_Utils_Tuple2(1320, 520)),
					nodeRadius: 20,
				...
				model.hovered,
				$author$project$Main$binaryTree);
		case 'OrderedTree':
			return A3(
				$author$project$Main$treeView,
				{
				...
				model.hovered,
				$author$project$Main$orderedTree);
		case 'FlareTree':
			return A3(
				$author$project$Main$loadedTreeView,
				{fixedSize: $elm$core$Maybe$Nothing, nodeRadius: 4, parentChildMargin: 130, peerMargin: 16, rotateNodeLabels: true, showLeafLabels: true},
				model.hovered,
				model.flare);
		default:
			return A3(
				$author$project$Main$loadedTreeView,
				{fixedSize: $elm$core$Maybe$Nothing, nodeRadius: 4, parentChildMargin: 130, peerMargin: 18, rotateNodeLabels: true, showLeafLabels: true},
				model.hovered,
				model.countries);
	}
};
var $author$project$Main$exerciseTitle = function (exercise) {
	switch (exercise.$) {
		case 'BinaryTree':
			return 'Aufgabe 9.1a: Binaerer Baum';
		case 'OrderedTree':
			return 'Aufgabe 9.1b: Allgemeiner geordneter Baum';
		case 'FlareTree':
			return 'Aufgabe 9.2: flare.json';
		default:
			return 'Aufgabe 9.3: Laenderhierarchie';
	}
};
				...
				[
					...
					_List_fromArray(
						[
							...
							_Utils_Tuple2(
							'tab-active',
							_Utils_eq(active, target))
						])),
					$elm$html$Html$Events$onClick(
					$author$project$Main$Select(target))
				]),
			_List_fromArray(
				[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A3($author$project$Main$tabButton, model.active, $author$project$Main$BinaryTree, '9.1 Binärer Baum'),
						A3($author$project$Main$tabButton, model.active, $author$project$Main$OrderedTree, '9.1 Geordneter Baum'),
						A3($author$project$Main$tabButton, model.active, $author$project$Main$FlareTree, '9.2 flare.json'),
						A3($author$project$Main$tabButton, model.active, $author$project$Main$CountryTree, '9.3 Länder')
					])),
				A2(
				$elm$html$Html$h2,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text(
						$author$project$Main$exerciseTitle(model.active))
					])),
				$author$project$Main$activeTreeView(model)
			]));
};
