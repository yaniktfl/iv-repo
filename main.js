...
var $author$project$Main$WithWeekends = {$: 'WithWeekends'};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Dict$RBEmpty_elm_builtin = {$: 'RBEmpty_elm_builtin'};
var $elm$core$Dict$empty = $elm$core$Dict$RBEmpty_elm_builtin;
var $author$project$Main$files = _List_fromArray(
	[
		_Utils_Tuple2('DJ', 'DJ.csv'),
		_Utils_Tuple2('NIKKEI', 'NIKKEI.csv'),
		_Utils_Tuple2('HANGSENG', 'HANGSENG.csv'),
		_Utils_Tuple2('DAX', 'DAX.csv'),
		_Utils_Tuple2('BOVESPA', 'BOVESPA.csv')
	]);
var $author$project$Main$GotCsv = F2(
	function (a, b) {
		return {$: 'GotCsv', a: a, b: b};
	});
...
var $author$project$Main$loadSeries = function (_v0) {
	var name = _v0.a;
	var file = _v0.b;
	return $elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString(
				$author$project$Main$GotCsv(name)),
			url: file
		});
};
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{failed: _List_Nil, loaded: $elm$core$Dict$empty, selected: 'DJ', weekendMode: $author$project$Main$WithWeekends},
		$elm$core$Platform$Cmd$batch(
			A2($elm$core$List$map, $author$project$Main$loadSeries, $author$project$Main$files)));
};
var $elm$core$Platform$Sub$batch = _Platform_batch;
var $elm$core$Platform$Sub$none = $elm$core$Platform$Sub$batch(_List_Nil);
var $author$project$Main$WithoutWeekends = {$: 'WithoutWeekends'};
...
var $author$project$Main$endDateExclusive = A2(
	$elm$core$Result$withDefault,
	$justinmimbs$date$Date$fromRataDie(1),
	$justinmimbs$date$Date$fromIsoString('2011-06-10'));
...
var $author$project$Main$startDate = A2(
	$elm$core$Result$withDefault,
	$justinmimbs$date$Date$fromRataDie(1),
	$justinmimbs$date$Date$fromIsoString('1980-12-23'));
...
var $author$project$Main$allDays = A2(
	$elm$core$List$map,
	$justinmimbs$date$Date$toIsoString,
	A4($justinmimbs$date$Date$range, $justinmimbs$date$Date$Day, 1, $author$project$Main$startDate, $author$project$Main$endDateExclusive));
...
var $author$project$Main$isTradingWeekday = function (date) {
...
var $author$project$Main$allWeekdays = A2(
	$elm$core$List$map,
	$justinmimbs$date$Date$toIsoString,
	A2(
		$elm$core$List$filter,
		$author$project$Main$isTradingWeekday,
		A4($justinmimbs$date$Date$range, $justinmimbs$date$Date$Day, 1, $author$project$Main$startDate, $author$project$Main$endDateExclusive)));
...
var $author$project$Main$decodeStockDay = A2(
	...
	A2(
		...
		A2($ericgj$elm_csv_decode$Csv$Decode$field, 'Date', $elm$core$Result$Ok)));
...
var $author$project$Main$csvStringToData = function (csvRaw) {
	return A2(
		$elm$core$Maybe$withDefault,
		_List_Nil,
		$elm$core$Result$toMaybe(
			A2(
				...
				$author$project$Main$decodeStockDay,
				$lovasoa$elm_csv$Csv$parse(csvRaw))));
};
...
var $author$project$Main$fillMissingDays = F2(
	function (targetDays, data) {
		var emptyDays = $elm$core$Dict$fromList(
			A2(
				$elm$core$List$map,
				function (date) {
				...
				targetDays));
		var actualDays = $elm$core$Dict$fromList(
			A2(
				$elm$core$List$filter,
				function (_v0) {
					var date = _v0.a;
					return A2($elm$core$Dict$member, date, emptyDays);
				},
				data));
		return $elm$core$Dict$toList(
			A2($elm$core$Dict$union, actualDays, emptyDays));
	});
	...
	function (msg, model) {
		switch (msg.$) {
			case 'GotCsv':
				...
				if (result.$ === 'Ok') {
					var csvRaw = result.a;
					var rawData = $author$project$Main$csvStringToData(csvRaw);
					var series = {
						calendarData: A2($author$project$Main$fillMissingDays, $author$project$Main$allDays, rawData),
						file: name + '.csv',
						name: name,
						rawCount: $elm$core$List$length(rawData),
						weekdayData: A2($author$project$Main$fillMissingDays, $author$project$Main$allWeekdays, rawData)
					};
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								loaded: A3($elm$core$Dict$insert, name, series, model.loaded)
							}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								failed: A2($elm$core$List$cons, name, model.failed)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'SelectSeries':
			...
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							weekendMode: function () {
								var _v2 = model.weekendMode;
								if (_v2.$ === 'WithWeekends') {
									return $author$project$Main$WithoutWeekends;
								} else {
									return $author$project$Main$WithWeekends;
								}
							}()
						}),
					$elm$core$Platform$Cmd$none);
...
var $author$project$Main$ToggleWeekendMode = {$: 'ToggleWeekendMode'};
...
var $author$project$Main$SelectSeries = function (a) {
	return {$: 'SelectSeries', a: a};
};
var $elm$virtual_dom$VirtualDom$text = _VirtualDom_text;
var $elm$html$Html$text = $elm$virtual_dom$VirtualDom$text;
var $author$project$Main$seriesButton = F2(
	function (model, _v0) {
		var name = _v0.a;
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Main$SelectSeries(name)),
					$elm$html$Html$Attributes$class(
					_Utils_eq(model.selected, name) ? 'active' : '')
				]),
			_List_fromArray(
				[
...
var $author$project$Main$RecordedData = F2(
	function (position, value) {
		return {position: position, value: value};
	});
var $author$project$Main$cellSize = 6;
...
var $author$project$Main$normalize = F2(
	function (values, value) {
		...
		if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
			var minValue = _v0.a.a;
			var maxValue = _v0.b.a;
			return _Utils_eq(minValue, maxValue) ? 0.5 : A3($elm$core$Basics$clamp, 0, 1, (value - minValue) / (maxValue - minValue));
		} else {
			return 0.5;
		}
	});
...
var $author$project$Main$valueColor = F2(
	function (values, maybeValue) {
		if (maybeValue.$ === 'Just') {
			var value = maybeValue.a;
			return $gampleman$elm_visualization$Scale$Color$viridisInterpolator(
				A2($author$project$Main$normalize, values, value));
		} else {
			return A3($avh4$elm_color$Color$rgb255, 188, 198, 190);
		}
	});
...
var $author$project$Main$drawCell = F2(
	function (values, recorded) {
		var _v0 = recorded.value;
		var date = _v0.a;
		var maybeValue = _v0.b;
		var label = date + (': ' + A2(
			$elm$core$Maybe$withDefault,
			'N.A.',
			A2($elm$core$Maybe$map, $elm$core$String$fromFloat, maybeValue)));
		return A2(
			$elm_community$typed_svg$TypedSvg$rect,
			_List_fromArray(
				[
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$x(recorded.position.x * $author$project$Main$cellSize),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(recorded.position.y * $author$project$Main$cellSize),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$width($author$project$Main$cellSize),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$height($author$project$Main$cellSize),
					$elm_community$typed_svg$TypedSvg$Attributes$fill(
					$elm_community$typed_svg$TypedSvg$Types$Paint(
						A2($author$project$Main$valueColor, values, maybeValue)))
				]),
			_List_fromArray(
				[
					A2(
...
var $author$project$Main$Level = F2(
	function (width, height) {
		return {height: height, width: width};
	});
var $author$project$Main$levels = _List_fromArray(
	[
		A2($author$project$Main$Level, 5, 1),
		A2($author$project$Main$Level, 1, 12),
		A2($author$project$Main$Level, 4, 1),
		A2($author$project$Main$Level, 2, 3),
		A2($author$project$Main$Level, 3, 3),
		A2($author$project$Main$Level, 1, 1)
	]);
var $author$project$Main$weekdayLevels = _List_fromArray(
	[
		A2($author$project$Main$Level, 5, 1),
		A2($author$project$Main$Level, 1, 9),
		A2($author$project$Main$Level, 4, 1),
		A2($author$project$Main$Level, 2, 3),
		A2($author$project$Main$Level, 3, 3),
		A2($author$project$Main$Level, 1, 1)
	]);
var $author$project$Main$levelsFor = function (mode) {
	if (mode.$ === 'WithWeekends') {
		return $author$project$Main$levels;
	} else {
		return $author$project$Main$weekdayLevels;
	}
};
var $elm$core$List$product = function (numbers) {
	return A3($elm$core$List$foldl, $elm$core$Basics$mul, 1, numbers);
};
var $author$project$Main$pixelHeightFor = function (mode) {
	return $elm$core$List$product(
		A2(
			...
			$author$project$Main$levelsFor(mode)));
};
var $author$project$Main$PixelPosition = F2(
	function (x, y) {
		return {x: x, y: y};
	});
...
var $author$project$Main$createPixelMap = function (remainingLevels) {
	if (!remainingLevels.b) {
		return _List_fromArray(
			[
				A2($author$project$Main$PixelPosition, 0, 0)
			]);
	} else {
		var level = remainingLevels.a;
		var rest = remainingLevels.b;
		var offsets = A2(
		...
		var innerWidth = $elm$core$List$product(
			A2(
				...
				rest));
		var innerHeight = $elm$core$List$product(
			A2(
				$elm$core$List$map,
				function ($) {
					return $.height;
				...
				rest));
		var inner = $author$project$Main$createPixelMap(rest);
		return A2(
			$elm$core$List$concatMap,
			function (_v1) {
				var localX = _v1.a;
				...
				return A2(
					$elm$core$List$map,
					function (position) {
						return {x: (localX * innerWidth) + position.x, y: (localY * innerHeight) + position.y};
					},
					inner);
			},
			offsets);
...
var $author$project$Main$pixelListFor = function (mode) {
	return $author$project$Main$createPixelMap(
		$author$project$Main$levelsFor(mode));
};
var $author$project$Main$pixelWidthFor = function (mode) {
	return $elm$core$List$product(
		A2(
			$elm$core$List$map,
			function ($) {
...
var $author$project$Main$viewSeries = F2(
	function (mode, series) {
		var minY = 0;
		var minX = 0;
		var maxY = $author$project$Main$pixelHeightFor(mode) - 1;
		var maxX = $author$project$Main$pixelWidthFor(mode) - 1;
		var data = function () {
			if (mode.$ === 'WithWeekends') {
				return series.calendarData;
			} else {
				return series.weekdayData;
			}
		}();
		var values = A2($elm$core$List$filterMap, $elm$core$Tuple$second, data);
		var currentPixelList = $author$project$Main$pixelListFor(mode);
		var drawnData = A3($elm$core$List$map2, $author$project$Main$RecordedData, currentPixelList, data);
		var mappingSize = $elm$core$List$length(currentPixelList);
		return A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							'Loaded: ' + (series.name + ('(' + ($elm$core$String$fromInt(
								$elm$core$List$length(data)) + ('), raw ' + ($elm$core$String$fromInt(series.rawCount) + (', missing ' + $elm$core$String$fromInt(
								$elm$core$List$length(data) - $elm$core$List$length(values)))))))))
						])),
					A2(
					$elm$html$Html$p,
					_List_fromArray(
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							'PixelMapping Size: ' + ($elm$core$String$fromInt(mappingSize) + (' Min X' + ($elm$core$String$fromInt(minX) + (' Max X' + ($elm$core$String$fromInt(maxX) + (' Min Y' + ($elm$core$String$fromInt(minY) + (' Max Y' + $elm$core$String$fromInt(maxY))))))))))
						])),
					A2(
					...
					_List_fromArray(
						[
							...
							$author$project$Main$pixelWidthFor(mode) * $author$project$Main$cellSize),
							$elm_community$typed_svg$TypedSvg$Attributes$InPx$height(
							$author$project$Main$pixelHeightFor(mode) * $author$project$Main$cellSize),
							A4(
					...
					_List_fromArray(
						[
							...
							A2(
								$elm$core$List$map,
								$author$project$Main$drawCell(values),
								drawnData))
						]))
				]));
	});
var $author$project$Main$view = function (model) {
	var maybeSeries = A2($elm$core$Dict$get, model.selected, model.loaded);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Übung 7: Recursive-Pattern-Technik')
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_Utils_ap(
					A2(
						$elm$core$List$map,
						$author$project$Main$seriesButton(model),
						$author$project$Main$files),
					_List_fromArray(
						[
							A2(
							$elm$html$Html$button,
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick($author$project$Main$ToggleWeekendMode),
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									function () {
										var _v0 = model.weekendMode;
										if (_v0.$ === 'WithWeekends') {
											return 'Wochenenden entfernen';
										} else {
											return 'Kalendertage zeigen';
										}
									}())
								]))
						]))),
				function () {
				if (maybeSeries.$ === 'Just') {
					var series = maybeSeries.a;
					return A2($author$project$Main$viewSeries, model.weekendMode, series);
				} else {
					return A2(
						$elm$html$Html$p,
						_List_fromArray(
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								A2($elm$core$List$member, model.selected, model.failed) ? ('Fehler beim Laden von ' + (model.selected + '.csv')) : 'Loading...')
							]));
				}
			}()
			]));
